...
function createRoom(name, variant = 'holdem', maxSeats = 6, roomId = null, isDefault = false) {
...
async function loadState() {
  ...
  } else if (globalThis.__JOSEPH_POKER_STATE__) {
    return normalizeState(globalThis.__JOSEPH_POKER_STATE__);
  }
...
async function saveState(state) {
  ...
  else globalThis.__JOSEPH_POKER_STATE__ = state;
}
...
function serialize(room, viewerToken = '') {
  const t = room.table;
  const viewerSeat = seatForToken(t, viewerToken);
  const actor = currentActorIndex(t);
  return {
    ...
    realtime_mode: hasRedis() ? 'vercel-redis-polling' : 'local-memory-polling',
  };
}
...
async function route(req, res, routeName) {
  try {
    ...
    if (routeName === 'rooms' && method === 'GET') {
      const state = await loadState();
      return json(res, 200, { rooms: Object.values(state.rooms).map(roomSummary), default_room_id: state.default_room_id });
    }
    ...
    if (routeName === 'rooms/create' && method === 'POST') {
      const out = await withState(state => {
        ...
        const room = createRoom(body.name || 'Poker Table', body.variant || 'holdem', body.max_seats || 6, id, false);
        state.rooms[id] = room;
        return { room_id: id, creator_token: room.creator_token };
      });
    ...
    if (routeName === 'state' && method === 'GET') {
      ...
      return json(res, 200, serialize(room, url.searchParams.get('token') || ''));
    }
    ...
    if (routeName === 'join' && method === 'POST') {
      const out = await withState(state => {
        ...
        if (body.ai_level === 'human' && body.client_id && t.seats.some(p => p && p.ai_level === 'human' && p.client_id === body.client_id)) throw new Error('You already have a seat at this table');
        const p = makePlayer(seat, body.name || `Player ${seat + 1}`, body.ai_level || 'human', body.client_id || null, t);
        t.seats[seat] = p;
        appendLog(t, 'System', `${p.player_name} joined seat ${seat + 1}`);
        return { seat_index: seat, token: p.token, room_id: room.room_id };
      });
      return json(res, 200, out);
    }
    ...
    if (routeName === 'add_ai' && method === 'POST') {
      const out = await withState(state => {
        ...
        t.seats[seat] = p;
        appendLog(t, 'System', `${p.player_name} joined seat ${seat + 1}`);
        return { seat_index: seat, token: null, room_id: room.room_id };
      });
      return json(res, 200, out);
    }
    ...
    if (routeName === 'leave' && method === 'POST') {
      const out = await withState(state => {
        ...
        if (!room.is_default && !iterPlayers(room.table).length) delete state.rooms[room.room_id];
        return { ok: true };
      });
      return json(res, 200, out);
    }
    ...
    if (routeName === 'start_hand' && method === 'POST') {
      const out = await withState(state => { const room = getRoom(state, body.room_id); startHand(room.table); return { ok: true }; });
      return json(res, 200, out);
    }

    if (routeName === 'next_hand' && method === 'POST') {
      const out = await withState(state => { const room = getRoom(state, body.room_id); nextHand(room.table); return { ok: true }; });
      return json(res, 200, out);
    }
    ...
    if (routeName === 'reset' && method === 'POST') {
      const out = await withState(state => { const room = getRoom(state, body.room_id); resetTable(room.table); return { ok: true }; });
      return json(res, 200, out);
    }

    if (routeName === 'set_variant' && method === 'POST') {
      const out = await withState(state => { const room = getRoom(state, body.room_id); configureVariant(room.table, body.variant || 'holdem'); return { ok: true }; });
      return json(res, 200, out);
    }
    ...
    if (routeName === 'variance/demo' && method === 'GET') {
      ...
      const agentB = url.searchParams.get('agent_b') || 'ai_easy';
