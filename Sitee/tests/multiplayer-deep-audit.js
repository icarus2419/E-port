...
(async () => {
  ...
  // Same browser/client cannot take a second seat; it reconnects to the existing seat instead.
  const duplicate = await must('POST', '/api/join', { room_id: roomId, seat_index: 0, name: 'Alice2', ai_level: 'human', client_id: 'alice-client' }, 'Alice duplicate reconnect');
  ok(duplicate.already_seated && duplicate.seat_index === 0 && duplicate.token === alice.token, 'duplicate same-client join should return the existing seat/token', duplicate);

  const reclaim = await must('GET', `/api/state?room_id=${roomId}&client_id=alice-client`, null, 'Alice token recovery by client id');
  ok(reclaim.viewer_token_valid && reclaim.viewer_seat === 0 && reclaim.recovered_token === alice.token, 'client id should recover the browser seat token', reclaim);
  ...
  console.log(JSON.stringify({
    ...
    checked: [
      ...
      'duplicate same-client reconnects instead of making a second seat', 'client id token recovery', 'private hole card visibility', 'start and complete two-human hand',
