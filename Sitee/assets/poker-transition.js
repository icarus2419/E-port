(() => {
  ...
  function canHandle(anchor, event) {
    if (!anchor || !anchor.href || anchor.target && anchor.target !== '_self') return false;
    if (anchor.hasAttribute('download') || isModifiedClick(event)) return false;
  ...
  function getOverlay() {
    ...
    chipLayer.append(makeChip(), makeChip(), makeChip());

    stage.append(table, cardLayer, chipLayer);
