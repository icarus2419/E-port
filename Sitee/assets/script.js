(() => {
  ...
  /* Featured video button:
     Click now triggers a clean black "iris-close" transition.
     If href is still "#", the transition closes then re-opens so the page does
     not get stuck. When you replace href="#" with the real 3D portfolio URL,
     the iris closes fully, then navigation happens on black. */
  ...
  const playIrisClose = (originElement, onClosed) => {
    ...
    animateIrisRadius(iris, maxRadius, 0, 860, () => {
      onClosed?.({
        reset: () => {
          window.setTimeout(() => {
            animateIrisRadius(iris, 0, maxRadius, 620, () => {
  ...
  document.querySelectorAll('.hero-video').forEach((btn) => {
    ...
    btn.addEventListener('click', (e) => {
      ...
      if (prefersReduced) {
        if (hasRealDestination) window.location.assign(href);
        return;
      }

      playIrisClose(btn, ({ reset }) => {
