(() => {
  ...
  // Safari/Chrome can restore this page from the back-forward cache exactly
  // as it looked when navigation started. If the user goes back from the 3D
  // portfolio, that can leave the black iris overlay closed over the homepage.
  // Always clear that transient navigation state when the page is shown again.
  ...
  const playIrisClose = (originElement, onClosed) => {
    ...
    // Hard failsafe: if the browser cancels/blocks the route change, never
    // leave the homepage covered by a closed black iris.
