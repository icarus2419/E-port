...
app.use(cors({
  origin(origin, cb) {
    ...
    // explicitly allow-listed origins, and any localhost/127.0.0.1 port for local demos.
    if (!origin || ALLOWED_ORIGINS.has(origin) || /^https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?$/.test(origin)) {
      return cb(null, true);
    }
    // Disallowed cross-origin: don't send CORS headers, but don't error the request.
    // (The browser still blocks genuine cross-origin reads; same-origin loads keep working.)
...
// The SecureDoc app (re-skinned) is served under /app; its assets resolve to /app/*.
app.use('/app', express.static(PUBLIC_DIR));
// The marketing landing rewritten to showcase SecureDoc is served at the site root.
...
// SecureDoc app shell for any /app or /app/* route.
app.get(/^\/app(\/.*)?$/, (_req, res) => {
  res.sendFile(path.join(PUBLIC_DIR, 'index.html'));
});

// Everything else falls through to the marketing landing.
app.get(/.*/, (_req, res) => {
