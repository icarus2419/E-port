...
function roomsPayload(state) {
  return Object.values(state.rooms || {}).map(roomSummary);
}
...
async function route(req, res, routeName) {
  try {
    ...
    if (routeName === 'start_hand' && method === 'POST') {
      const out = await withState(state => { const room = getRoom(state, body.room_id); touchRoom(room); startHand(room.table); return mutationPayload(state, room, body.token || '', body.client_id || ''); });
      return json(res, 200, out);
    }

    if (routeName === 'next_hand' && method === 'POST') {
      const out = await withState(state => { const room = getRoom(state, body.room_id); touchRoom(room); nextHand(room.table); return mutationPayload(state, room, body.token || '', body.client_id || ''); });
      return json(res, 200, out);
    }
    ...
    if (routeName === 'reset' && method === 'POST') {
      const out = await withState(state => { const room = getRoom(state, body.room_id); touchRoom(room); resetTable(room.table); return mutationPayload(state, room, body.token || '', body.client_id || ''); });
      return json(res, 200, out);
    }

    if (routeName === 'set_variant' && method === 'POST') {
      const out = await withState(state => { const room = getRoom(state, body.room_id); touchRoom(room); configureVariant(room.table, body.variant || 'holdem'); return mutationPayload(state, room, body.token || '', body.client_id || ''); });
      return json(res, 200, out);
    }
