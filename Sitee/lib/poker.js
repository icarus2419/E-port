      ...
      const out = await withState(state => {
        ...
        if (body.ai_level === 'human' && clientId) {
          ...
          if (existingSeat != null) {
            ...
            return { seat_index: existingSeat, token: existing.token, room_id: room.room_id, already_seated: true };
          }
        }
    ...
    if (routeName === 'auto_join' && method === 'POST') {
      const out = await withState(state => {
        ...
        if (seat != null) {
          ...
          return { seat_index: seat, token: existing.token, room_id: room.room_id, already_seated: true };
        }
