  ...
  try {
    ...
    if (routeName === 'rooms/create' && method === 'POST') {
      const out = await withState(state => {
        ...
        state.rooms[id] = room;
        return { room_id: id, creator_token: room.creator_token, warning: hasRedis() ? '' : 'Temporary server storage is active; add Redis env vars or this room may disappear on Vercel.' };
      });
    ...
    if (routeName === 'state' && method === 'GET') {
      const out = await withState(state => {
        ...
        touchRoom(room);
        return serialize(room, url.searchParams.get('token') || '');
      });
      return json(res, 200, out);
    }
    ...
    if (routeName === 'start_hand' && method === 'POST') {
      const out = await withState(state => { const room = getRoom(state, body.room_id); touchRoom(room); startHand(room.table); return { ok: true }; });
      return json(res, 200, out);
    }

    if (routeName === 'next_hand' && method === 'POST') {
      const out = await withState(state => { const room = getRoom(state, body.room_id); touchRoom(room); nextHand(room.table); return { ok: true }; });
      return json(res, 200, out);
    }
    ...
    if (routeName === 'reset' && method === 'POST') {
      const out = await withState(state => { const room = getRoom(state, body.room_id); touchRoom(room); resetTable(room.table); return { ok: true }; });
      return json(res, 200, out);
    }

    if (routeName === 'set_variant' && method === 'POST') {
      const out = await withState(state => { const room = getRoom(state, body.room_id); touchRoom(room); configureVariant(room.table, body.variant || 'holdem'); return { ok: true }; });
      return json(res, 200, out);
    }
