...
function updateCallMatchBanner(p, legalDetails) {
  ...
  if (!banner) return;
  banner.hidden = true;
  banner.className = 'call-match-banner';
  banner.innerHTML = '';
  ...
  banner.hidden = false;
  if (toCall > 0) {
    banner.classList.add('call-due');
    banner.innerHTML = `<strong>To call: ${escapeHtml(toCall)} chips</strong><span>${isMyTurn ? 'Pay this to match the current bet.' : 'Needed to match the current bet.'}</span>`;
  } else {
    banner.classList.add('call-clear');
    banner.innerHTML = `<strong>You are matched</strong><span>0 chips needed to call.</span>`;
  }
}
